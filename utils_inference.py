...
"""
Tiled Inference Utilities - OpenMats Support Module

Handles image tiling, ONNX-based inference, and result merging for normal map
generation from color/albedo inputs. Used to support tile-based inference for 
large images with overlap blending.

Includes:
- tiles_split(): Splits images into overlapping tiles with padding
- tiles_infer(): Runs inference on each tile using ONNX
- tiles_merge(): Reconstructs the final image from tiles using blending masks
- normalize(): Normalizes output to unit-length vectors

Corner and mask blending logic adapted from HugoTini (GitHub).

Used internally by OpenMats to generate seamless normal maps.
"""
