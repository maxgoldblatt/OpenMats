...
"""
DeepBump Inference Wrapper - OpenMats Utility Module

Runs an ONNX model to generate a normal map from a color (albedo) image using
tile-based inference with overlap control. Includes:

- Grayscale conversion and tiling of input image
- ONNX runtime model loading and tile-wise prediction
- Normal map reconstruction and normalization

Originally adapted from HugoTini's DeepBump repository (GitHub).
Used internally in OpenMats for high-fidelity normal map generation.

Note: Requires `deepbump256.onnx` in the module directory.
"""
