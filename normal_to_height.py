...
"""
Normal Map to Height Map Conversion - OpenMats Utility Module

Implements gradient processing and the Frankot-Chellappa algorithm to reconstruct
a height map from a normal map. Includes:

- normal_to_grad(): Converts normal vector to gradient
- copy_flip(): Wraps gradient maps by tiling and flipping
- frankot_chellappa(): Computes height map from gradients using FFT

Adapted from work by HugoTini (GitHub). Used internally by OpenMats for
converting AI-generated normals into usable displacement maps.
"""
